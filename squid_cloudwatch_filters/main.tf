...
The outbound proxy has denied outbound requests.
...
resource "aws_cloudwatch_metric_alarm" "squid_denied_alarm_critical" {
  alarm_name        = "${var.env_name}-squid-denials-critical"
  alarm_description = <<EOM
The outbound proxy has denied too many outbound requests.

Runbook: ${var.runbook_url}

Notifying: ${join(", ", var.slack_handles)}
EOM

  namespace   = var.metric_namespace
  metric_name = "${var.env_name}/DeniedRequests"

  # alert when sum(denials) >= 25 for any 1 minute out of 15 eval periods
  statistic           = "Sum"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  threshold           = 25
  period              = 60
